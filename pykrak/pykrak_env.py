...
class Env:
    ...
    """
    def __init__(self, z_list, cp_list, cs_list, rho_list, attnp_list, attns_list, cp_top, cs_top, rho_top, attnp_top, attns_top, cp_bott, cs_bott, rho_bott, attnp_bott, attns_bott, attn_units):
        self.z_list = z_list
    ...
    def get_modes(self, freq, Ng_list=[], rmax=0.0, c_low=0.0, c_high=1e10):
        ...
        krs, z, phi, ugs = kr.list_input_solve(freq, self.z_list, self.cp_list, self.cs_list, self.rho_list, self.attnp_list, self.attns_list, self.cp_top, self.cs_top, self.rho_top, self.attnp_top, self.attns_top, self.cp_bott, self.cs_bott, self.rho_bott, self.attnp_bott, self.attns_bott, self.attn_units, Ng_list, rmax, c_low, c_high)
    ...
    def plot_env(self, ax=None, color=None):
        ...
        for i in range(len(self.z_list)):
            ax.hlines(self.z_list[i][-1], min([x.min() for x in self.cp_list]), max([x.max() for x in self.cp_list]), 'k', alpha=0.5)
            rhoax.hlines(self.z_list[i][-1], min([x.min() for x in self.rho_list]), max([x.max() for x in self.rho_list]), 'k', alpha=0.5)
            attnax.hlines(self.z_list[i][-1], min([x.min() for x in self.attnp_list]), max([x.max() for x in self.attnp_list]), 'k', alpha=0.5)
        ax.hlines(self.z_list[-1][-1], min([x.min() for x in self.cp_list]), max([x.max() for x in self.cp_list]), 'k')
        rhoax.hlines(self.z_list[-1][-1], min([x.min() for x in self.rho_list]), max([x.max() for x in self.rho_list]), 'k')
        attnax.hlines(self.z_list[-1][-1], min([x.min() for x in self.attnp_list]), max([x.max() for x in self.attnp_list]), 'k')
        mean_layer_c = sum([x.mean() for x in self.cp_list]) / len(self.cp_list)
        ax.text(mean_layer_c , self.z_list[-1][-1] +30 ,  '$c_b$:{0}, \n$\\rho_b$:{1}, \n$\\alpha_b$:{2}'.format(self.cp_bott, self.rho_bott, self.attnp_bott))
        return

class FluidEnv(Env):
    def __init__(self, z_list, cp_list, rho_list, attnp_list, cp_top, rho_top, attnp_top, cp_bott, rho_bott, attnp_bott, attn_units):
        ...
        super().__init__(z_list, cp_list, cs_list, rho_list, attnp_list, attns_list, cp_top, cs_top, rho_top, attnp_top, attns_top, cp_bott, cs_bott, rho_bott, attnp_bott, attns_bott, attn_units)
