...
for iset, fact in enumerate(Nv):
    ...
    alpha_arr = np.concatenate((0.0* np.ones(N_list[0]), 0.0* np.ones(N_list[1]), 0.0 * np.ones(N_list[2])))
    alphas_arr = np.concatenate((0.0* np.ones(N_list[0]), 0.0* np.ones(N_list[1]), 0.0 * np.ones(N_list[2])))
