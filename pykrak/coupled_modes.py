...
def get_on_new_grid(z0, z1, rho0, rho1, rho_hs0, rho_hs1, phi0, phi1, gammas0, gammas1):
    ...
    if Z1 > Z0:  # need to extend modes from z0
        ...
        new_z0 = np.concatenate(
            (z0, z_extra)
        )  # be careful here because arrays are mutable...
...
def update_a0(
    ...
    if not same_grid:  # need to interpolate
        z1, rho0, phi0, rho1, phi1 = get_on_new_grid(
            z0, z1, rho0, rho1, rho_hs0, rho_hs1, phi0, phi1, gammas0, gammas1
        )
    Z1_new = np.max(z1)
    a0_adv = advance_a(a0, krs0, r0, r1)  #
    ...
    tail_values_weighted = (
        a0_adv_weighted * phi0[-1, :] * np.exp(-gammas0 * (Z1_new - Z0))
    )
    ...
    for l in range(M1):
        ...
        tail1 = (
            ...
            / rho_hs1
            * phi1_l[-1]
            * np.exp(-gammas1[l] * (Z1_new - Z1))
            * np.sum(tail_values / (gammas1[l] + gammas0))
        )
        term1 += tail1
        ...
        tail2 = (
            ...
            * phi1_l[-1]
            * np.exp(-gammas1[l] * (Z1_new - Z1))
            * np.sum(tail_values_weighted / (gammas1[l] + gammas0))
        )
        term2 += tail2
        term2 /= krs1[l]
...
def compute_cm_pressure(
    ...
    for i in range(1, rgrid.size):
        ...
        if rs <= r1:  # source is in this segment
            ...
            if (
                i == 1
            ):  # first segment is special case (phase of a0 is for source-receiver range equal to the first segment length r1)
                a0 = update_a0(
                    a0,
                    krs0,
                    gammas0,
    ...
    p /= np.sqrt(rs)
    p_zr_real = interp.vec_lin_int(
        zr, zgrid1, p.real
    )  # zgrid should be very fine so I don't see any issue here...
    p_zr_imag = interp.vec_lin_int(zr, zgrid1, p.imag)
    p_zr = p_zr_real + 1j * p_zr_imag
    return p_zr


def compute_arr_cm_pressure(
    omega,
    krs_list,
    phi_list,
    zgrid_list,
...
def downslope_coupled_mode_code():
    ...
    for Z in Zvals:
        ...
        c_list = [np.array([1500.0, 1470.0]), np.array([1800.0, 1800.0])]
        env_rho_list = [np.array([1.0, 1.0]), np.array([rho_hs, rho_hs])]
        attn_list = [np.array([0.0, 0.0]), np.array([attn_hs, attn_hs])]
        cmin = min([c_list[i].min() for i in range(len(c_list))])
        cmax = 1799.0

        N_list = [
            max(int(np.ceil((z_list[i][1] - z_list[i][0]) / dz)), 4)
    ...
    zr = np.linspace(5.0, Zvals.max(), 200)
