...
@njit(cache=True)
def get_layered_eta_aiaj_integrator(
    h_arr, ind_arr, z_arr, c_arr, ci_arr, cj_arr, rho_arr
):
    """
...
def get_zfg_eta_ai(h_arr, ind_arr, ci_arr, z_arr, c_arr, rho_arr, krs, phi, omega):
    ...
    integrator = get_layered_eta_ai_integrator(
        h_arr, ind_arr, z_arr, c_arr, ci_arr, rho_arr
    )
...
@njit(cache=True)
def get_zfg_eta_aiaj(
    h_arr, ind_arr, ci_arr, cj_arr, z_arr, c_arr, rho_arr, krs, phi, omega
):
    """
    ...
    """
    M = krs.size  # number of modes
    integrator = get_layered_eta_aiaj_integrator(
        h_arr, ind_arr, z_arr, c_arr, ci_arr, cj_arr, rho_arr
    )
    zfg_arr = np.zeros((M, M))
    ...
    for f in range(M):
        ...
        for g in range(f, M):
            ...
            if f == g:
                rfg = (
                    -1j * r * H0f / (2 * krf)
                )  # minus sign because of different FFT convention used compared to T and K
                rfg_arr[f, g] = rfg
            else:
                numer = H0f - H0(krg, r)
                denom = krf**2 - krg**2
...
def get_dpdaidaj(zfg_ai, zfg_aj, zfg_aiaj, krs, r, phi_zr, phi_zs):
    ...
    for f in range(M):
        for g in range(M):
            ...
            term = 0.5 * zfg_aiaj[f, g] * rfg[f, g] * phi_zr[:, f] * phi_zs[:, g]
            dpdaidaj += term
    dpdaidaj *= 0.5 * 1j
    return dpdaidaj
    ...
    for k in range(P):
        ...
        for zs_i in range(num_zs):
            ...
            for r_i in range(num_r):
                dpda[zs_i, :, r_i, k] = get_dpda(
                    zfg_k, krs, r_arr[r_i], phi_zr, phi_zs_i
                )
    return dpda
...
def get_mode_dpda_arr(zfg_arr, krs, r_arr, phi_zs):
    ...
    for k in range(P):
        ...
        for zs_i in range(num_zs):
            ...
            phi_zs_i = np.reshape(phi_zs_i, (1, phi_zs_i.size))
            for r_i in range(num_r):
                mode_dpda[:, zs_i, r_i, k] = get_mode_dpda(
                    zfg_k, krs, r_arr[r_i], phi_zs_i
                )
    return mode_dpda
...
@njit(cache=True)
def get_kernel(
    h_arr, ind_arr, z_arr, c_arr, rho_arr, krs, phi_z, phi, omega, zr, zs, rgrid, stride
):
    """
    Get VTSK
    Input
    ...
    for j in range(zr.size):
        ...
        for f in range(M):
            ...
            krf = krs[f]
            kernel_j += np.outer(
                -1j * rgrid * H0(krf, rgrid),
                uf**2 * phi_zr[j, f] * phi_zs[0, f] / (2 * krf),
            )
            for g in range(M):
                krg = krs[g]
                ug = phi[::stride, g]
                if g == f:
                    ...
                    kernel_j += np.outer(
                        H0(krf, rgrid) - H0(krg, rgrid),
                        uf * ug * phi_zr[j, f] * phi_zs[:, g] / (krf**2 - krg**2),
                    )
                # integrand = uf * ug
